...
// On défini un nouvel array contenant tous les livres (l'original sera modifié)
let books2 = []
...
// Même fonction de comparaison, mais sur les titres
...
// Fonction trouvant l'index d'un livre en fonction d'un id qu'on lui passe
...
// On utilise .find et une arrow function pour trouver un élément par son id
let findId = books.find(item => item.id == 873495);
// On utilise .every avec rented supérieur à 0, retourne vrai si la valeur est supérieure à 0
let rent = books.every(item => item.rented > 0)
// On tri les livres du moins loué au plus loué
let mostRented = books.sort(compare)
// On extrait le 1er livre de ce nouvel array (le moins loué)
let lessRented = mostRented.shift()
// On extrait le dernier livre de ce nouvel array (le plus loué)
mostRented = mostRented.pop()

// On utilise splice avec la fonction detect sur l'array books2 non modifié pour supprimer le livre ayant 133712 comme id
...
// On utilise ort avec la fonction compareAlpha sur l'array books2 pour le trier
